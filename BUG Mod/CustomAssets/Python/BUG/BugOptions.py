	...
	def read(self):
		try:
			...
			if not self.path:
				...
				file.write("# Generated by BUG Mod.\n\n")
				file.close()
...
#			BugConfigTracker.add("BUG_Mod_Config", self.path)
			...
			self.path = None
			self.config = None
			BugUtil.debug("BUG: File not found: %s" % name)
		...
		if self.isLoaded():
			if self.isDirty():
				try:
					...
					BugUtil.debug("BUG: Failed writing to INI file %s" % name)
		else:
			BugUtil.debug("BUG: INI file wasn't read")
